...
int start_bf_compiler(int sim_argc, char **sim_argv) {
    ...
    if ((in = fopen(sim_argv[1],"r"))==NULL) {
        printf("Unable to open file '%s'",sim_argv[1]);
        return 1;
    }

    if ((out = fopen(sim_argv[2],"wb+"))==NULL) {
        printf("Unable to open file '%s' (still running?)",sim_argv[2]);
        return 1;
    }
